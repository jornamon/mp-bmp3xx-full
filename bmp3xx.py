...
class RingBuffer:
    ...
    def put(self, data):
        ...
        if self._write_index == self._read_index:  # No more room in the queue
            self._read_index = (self._read_index + 1) % self._max_size  # Discard old data increasing read pointer too
...
class BMP3XXFIFO():
    ...
    def __init__(self, sensor: BMP3XX, max_frames = 100, enable_alt: bool = True) -> None:
        ...
        self._sensor.config_write(print_result=False,
            mode="normal",
            temp_en=1,
            press_en=1,
            fifo_mode=1,
        ...
        self._sensor._fifo_auto_queue = self  # Update reference to this FIFO queue in the sensor
    ...
    def get(self):
        ...
        if (current_size == 0 and time_since_last_feed > self._fifo_odr) or (
            current_size < self._feed_threshold and time_since_last_feed > self._fifo_odr * self._feed_threshold / 2
        ):
            self.feed_queue()
...
class BMP3XX(Sensor):
    ...
    def __init__(self, bus, debug_print=False, **kwargs) -> None:
        ...
        self._endianness: Literal['big', 'little'] = "little"  # To pack and unpack bytes <-> ints
        ...
        self._fifo_auto_queue: BMP3XXFIFO | None = None  # Reference to BMP3XXFIFO object if exists
        ...
        self.apply_config_preset("init")  # Basic config, bmp3xx boots up with sensors disabled.
    ...
    def fifo_debug(self, num_bytes: int = 0) -> None:
        ...
        legend = OrderedDict()
        stats = OrderedDict()
        legend.update({d[0]: d[1] for d in ((frame.name, frame.representation) for frame in self._sensor_frames.values())})
        stats.update({key: 0 for key in (frame.name for frame in self._sensor_frames.values())})
        last_byte = self._fifo_sync(num_bytes)
        stats.update({'INVALID': 0, 'TOTAL ERRORS': 0, 'TOTAL FRAMES': 0, 'TOTAL BYTES': last_byte})
        i =  0
    ...
    def fifo_read(self, num_bytes: int = 0) -> Generator:
        ...
        while i < last_byte:
            ...
            if header in self._sensor_frames:
                ...
                fc = BMP3XX.frame_content(frame.name, frame_payload.popitem()[1])  # Just take the value, not the dict
                i += frame.size_bytes
            else:
                #Unrecognized frame
    ...
    def calc_odr(self, explain = True, **kwargs) -> tuple[float, Any]:
        ...
        if not kwargs:
            config = self.config_read('press_en', 'temp_en', 'osr_p', 'osr_t', 'odr_sel', 'fifo_subsampling')
        ...
        conversion_time_ms = 0.234 + press_en*(0.392 + osr_p*2.020) + temp_en*(0.163 + osr_t*2.020)
    ...
    def calibrate_altimeter(self, **calib_info) -> None:  # type: ignore
        ...
        if len(calib_info) == 1 and any(('local_alt' in calib_info, 'local_press' in calib_info)):
            ...
            raise SensorError("To calibrate the altimeter you must provide one and only one of these keyword arguments: \n"
